...
const screenForNames = (arr) => arr.filter(name => name.match(/^(?:Mrs|Mr|Dr|Ms)+.(?:\s)[A-Za-z]+/g));
...
const toTitleCase = (arr) => arr.map(str => str[0].toUpperCase() + str.substring(1));
...
let biggerThanLuke = (arr) => {
  let massArr = arr.filter(character => character.mass > 77);
  let newArr = massArr.map(character => character.name);
...
const detectTicTacToeWin = (board) => {
  const winCon = (row1, col1, row2, col2, row3, col3) => {
  ...
  if(winCon(0,0,0,1,0,2)) return true;
  if(winCon(1,0,1,1,1,2)) return true;
  if(winCon(2,0,2,1,2,2)) return true;

  if(winCon(0,0,1,0,2,0)) return true;
  if(winCon(0,1,1,1,2,1)) return true;
  if(winCon(0,2,1,2,2,2)) return true;

  if(winCon(0,0,1,1,2,2)) return true;
  if(winCon(0,2,1,1,2,0)) return true;
